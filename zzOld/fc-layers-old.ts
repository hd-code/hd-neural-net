...
import { ACT_FUNC, applyToVector, isActivationFunction } from "../src/activation-functions";
    ...
    hiddenLayers ?:IFCLayerConfig[], noBias ?:boolean) :INet
{
    ...
    if (hiddenLayers) {
        result = hiddenLayers.map((layer, i) => {
            ...
            return <IFCLayer>{
                actFunction: layer.actFunction || ACT_FUNC.Sigmoid,
    ...
    result.push(<IFCLayer>{
        actFunction: outputLayer.actFunction || ACT_FUNC.RectifiedLinear,
...
export { ACT_FUNC as EActFunction } from '../src/activation-functions'
