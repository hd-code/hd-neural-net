...
import * as p from '../src/perceptron';
...
const cases = [{
    ...
    data: [
        { input: [0,0], expected: 0, result: 0, error: 0, deltaW: [ 0,0], deltaB: 0 },
        { input: [0,1], expected: 0, result: 0, error: 0, deltaW: [ 0,0], deltaB: 0 },
        { input: [1,0], expected: 0, result: 1, error:-1, deltaW: [-1,0], deltaB:-1 },
        { input: [1,1], expected: 1, result: 1, error: 0, deltaW: [ 0,0], deltaB: 0 },
    ]
}, {
    name: 'NOR perceptron',
    learningRate: 0.1,
    perceptron: { bias: -0.5, weights: [1.2,0.3] },
    data: [
        { input: [0,0], expected: 1, result: 0, error: 1, deltaW: [ 0, 0], deltaB: 1 },
        { input: [0,1], expected: 0, result: 0, error: 0, deltaW: [ 0, 0], deltaB: 0 },
        { input: [1,0], expected: 0, result: 1, error:-1, deltaW: [-1, 0], deltaB:-1 },
        { input: [1,1], expected: 0, result: 1, error:-1, deltaW: [-1,-1], deltaB:-1 },
    ]
}, {
    name: 'only middle value perceptron',
    learningRate: 0.1,
    perceptron: { bias: 0, weights: [-.7,.2,.4] },
    data: [
        { input: [0,0,0], expected: 0, result: 1, error:-1, deltaW: [ 0, 0, 0], deltaB:-1 },
        { input: [0,0,1], expected: 0, result: 1, error:-1, deltaW: [ 0, 0,-1], deltaB:-1 },
        { input: [0,1,0], expected: 1, result: 1, error: 0, deltaW: [ 0, 0, 0], deltaB: 0 },
        { input: [0,1,1], expected: 1, result: 1, error: 0, deltaW: [ 0, 0, 0], deltaB: 0 },
        { input: [1,0,0], expected: 0, result: 0, error: 0, deltaW: [ 0, 0, 0], deltaB: 0 },
        { input: [1,0,1], expected: 0, result: 0, error: 0, deltaW: [ 0, 0, 0], deltaB: 0 },
        { input: [1,1,0], expected: 1, result: 0, error: 1, deltaW: [ 1, 1, 0], deltaB: 1 },
        { input: [1,1,1], expected: 1, result: 0, error: 1, deltaW: [ 1, 1, 1], deltaB: 1 },
    ]
}];
...
describe('Perceptron', () => {
    describe(p.isPerceptron.name + '()', () => {
        [{
            name: 'normal perceptron', expected: true,
            perceptron: { bias: 0.5, weights: [0.3, 0.6] }
        }, {
            name: 'bias missing', expected: false,
            perceptron: { weights: [0.3, 0.6] }
        }, {
            name: 'bias not a number', expected: false,
            perceptron: { bias: '0.5', weights: [0.3, 0.6] }
        }, {
            name: 'weights missing', expected: false,
            perceptron: { bias: 0.5 }
        }, {
            name: 'weights no vector', expected: false,
    ...
    describe(p.train.name + '()', () => cases.forEach(({name, learningRate, perceptron, data}) => {
        it(name, () => data.forEach(d => {
            ...
            const actual = p.train(perceptron, d.input, d.expected, learningRate);
            assert.deepStrictEqual(actual, expected);
        }));
    }));
    ...
    describe(p.trainBatch.name + '()', () => cases.forEach(({name, learningRate, perceptron, data}) => {
        it(name, () => {
            ...
            const actual = p.trainBatch(perceptron, input, expOutput, learningRate);
